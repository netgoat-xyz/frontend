...
export function ReverseProxiesClient({ teamSlug, domainId, reverseProxies }: { teamSlug: string; domainId: string; reverseProxies: any[] }) {
  ...
  const handleAdd = async () => {
    if (!name || !target) return toast.error("Name and Target URL are required");
    setLoading(true);
    ...
    if (res.success) {
       ...
       toast.error(res.error || "Failed to add reverse proxy");
    }
    setLoading(false);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <Plus className="w-4 h-4" /> Add Upstream Target
        </button>
      </div>
      ...
      {adding && (
         <div className="p-4 border border-neutral-200 dark:border-neutral-800 rounded-xl bg-neutral-50/50 dark:bg-neutral-900/50 space-y-4 mb-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label className="text-xs font-medium text-neutral-500 mb-1 block">Rule Name</label>
                    <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. API Target" className="w-full bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-md px-3 py-2 text-sm" />
                </div>
                <div>
                    <label className="text-xs font-medium text-neutral-500 mb-1 block">Route Path</label>
                    <input value={path} onChange={e => setPath(e.target.value)} placeholder="e.g. /api/*" className="w-full bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-md px-3 py-2 text-sm font-mono" />
                </div>
                <div>
                    <label className="text-xs font-medium text-neutral-500 mb-1 block">Target URL</label>
                    <input value={target} onChange={e => setTarget(e.target.value)} placeholder="e.g. https://api.mybackend.com" className="w-full bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-md px-3 py-2 text-sm font-mono" />
                </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
                 <button onClick={() => setAdding(false)} className="px-4 py-2 text-sm text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300">Cancel</button>
                 <button onClick={handleAdd} disabled={loading} className="px-4 py-2 text-sm bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 rounded-md disabled:opacity-50">Save Rule</button>
            </div>
         </div>
      )}
      ...
      {reverseProxies && reverseProxies.length > 0 ? (
        ...
        <div className="py-12 flex flex-col items-center text-center text-neutral-500 space-y-4">
           <p className="text-sm">No reverse proxy rules configured. Traffic routes to the default target.</p>
        </div>
      )}
    </div>
  );
