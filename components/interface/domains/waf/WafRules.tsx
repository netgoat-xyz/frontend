...
export function WafRules({ rules = fallbackRules }: { rules?: WAFRule[] }) {
  return (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-neutral-500">
                  <span className="px-1.5 py-0.5 bg-neutral-800 border border-neutral-700 rounded">{rule.type || 'Custom'}</span>
                  <span className="ml-2">{rule.rules ?? 1} rules</span>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-md border ${
                (rule.status || (rule.enabled === false ? 'paused' : 'active')) === "active"
